<template>
    ...
    <div class="py-2 px-3 mr-2 bg-white border border-gray-200 rounded-lg dark:bg-neutral-900 dark:border-neutral-700"
        ...
        <div class="w-full flex flex-col justify-center items-center gap-x-3">
            ...
            <div class="flex">
                <input v-model="model"
                       ...
                       style="-moz-appearance: textfield;" type="number" disabled>
                <div v-if="value">
...
const onClickPlus = () => {
    if(model.value < props.maxValue){
        model.value = model.value + parseInt(props.stepValue);
    }
}

const onClickMinus = () => {
    if (model.value > 0 && model.value > props.minValue) {
        model.value = model.value - parseInt(props.stepValue);
    }
};
</script>
